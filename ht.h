...
public:
    ...
    HASH_INDEX_T next() 
    {
			...
			HASH_INDEX_T curr = (this -> start_ + (this -> numProbes_ )* dhstep_) % (this -> m_);
			this -> numProbes_++;
			return curr;
    }
};
...
{
    if (static_cast<double>(size_ + mCapacity_)/(CAPACITIES[mIndex_]) >= resizeAlpha_){
			resize();
		}
...
{
	HashItem* newitem = internalFind(key);
	if (newitem == NULL){
		return;
	}
	else{
	...
	for (size_t i = 0; i < temp.size(); i++){
		if (temp[i] != NULL){
			if(temp[i] -> deleted == false){
				size_t curr = probe(temp[i] -> item.first);
				if (curr != npos){
					table_[curr] = temp[i];
					size_++;
				}
